 ...
 *       doc: {
 *         asideTabs: {
 *           tree: { name: "目录", component: "VPJArticleAsideTreePage" },
 *           resources: { name: "资源", component: "VPJArticleAsideResourcesPage" }
 *         },
 *         cover: "/cover.jpg",
...
export interface VPJDocLayoutConfig {
     ...
     * - `"VPJArticleAsideOutlinePage"`，`"VPJArticleAsideTreePage"` 与 `"VPJArticleAsideResourcesPage"` 是默认的内置组件名称，即使你已经在全局注册了组件，也会被主题内置组件覆盖。
     * 
     * @example
     * 示例1：替换默认目录组件
     * ```ts
