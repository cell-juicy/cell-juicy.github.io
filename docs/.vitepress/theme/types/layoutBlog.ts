...
export interface VPJBlogLayoutConfig {
    ...
    /**
     * 布局级「上一页」按钮提示文本配置（博客布局）
     * @optional
     *
     * @remarks
     * 用于为所有博客页面配置默认上一页按钮提示文本。
     *
     * 配置规则：
     * - `string` → 使用该值作为提示文本
     * - `false` → 隐藏提示文本
    ...
    /**
     * 布局级自动生成上一页/下一页链接（博客布局）
     * @optional
     *
     * @remarks
     * 控制是否为所有博客页面自动生成 `next` / `prev` 链接。
     * 自动生成规则：
     * - 基于系列文章，按照 `order` 从小到大生成前一页/后一页
     *
     * 注意事项：
