...
##### 8.5.15 设$A$和$B$是两个非空的集合，其中$A$的基数不小于或者等于$B$的基数。利用超限归纳原理证明$B$的基数小于或者等于$A$的基数<span style="color:blue">（提示：对任意的子集 $X\subseteq B$ ，设$P(X)$表示性质：存在一个从$X$到$A$的单射）</span>。本题<span style="color:blue">（结合[习题8.3.3](../Chap8/Sec3.md)）</span>表明只要选择公理成立，任意两个集合的基数就是可比较的
...
##### 8.5.20 设$X$是一个集合。$\Omega\subset 2^X$是由$X$的子集构成的集合，并且设$\Omega$中不包括空集$\varnothing$ 。利用佐恩引理证明：存在子集$\Omega'\subseteq\Omega$使得$\Omega'$中所有的元素互不相交<span style="color:blue">（即如果$A$和$B$是$\Omega'$中不同的元素，那么$A\cap B=\varnothing$）</span>，而且$\Omega$中的每个元素都至少与$\Omega'$中的一个元素相交<span style="color:blue">（即定义任意的$C\in \Omega$，存在一个$A\in\Omega'$使得$C\cap A\ne \varnothing$）</span>。<span style="color:blue">（提示：考虑由 $\Omega$ 的满足条件“全体元素都互不相交”的所有子集所构成的整体，并找到这个整体的一个最大元素）</span>反过来，如果上述结论成立，证明：它蕴含了[习题8.4.2](../Chap8/Sec4.md)的结论，从而这是另一个在逻辑上与选择公理等价的结论<span style="color:blue">（提示：设$\Omega$是由所有形如$\{(0,\alpha),(1,x_\alpha)\}$的序对集构成的集合，其中$\alpha \in I$且$x_\alpha\in X_\alpha$）</span>
