...
6. <span style="color:red">（15.6.10 复数的绝对值）</span>设$z=a+b\text{i}$是一个复数，其中$a$，$b$都是实数，那么$z$的**绝对值**$|z|$为实数$|z|:=\sqrt{a^2+b^2}=(a^2+b^2)^{1/2}$。
   ...
   <span style="color:blue">（注：在[习题5.6.3](./Sec6.md)中我们提到了绝对值的另一个表达式$|x|=\sqrt{x^2}$，定义15.6.10正是这个式子的推广）</span>
...
##### 15.6.14 证明引理15.6.14<span style="color:blue">（提示：分别把$z_n$和$w_n$划分成实部和虚部，然后使用通常的极限定律（[引理6.1.19](./Sec6.md)）和引理15.6.13）</span>
...
[实分析 5.6 实数的指数运算 I](./Sec6.md)

[实分析 6.1 收敛与极限定律](./Sec6.md)
