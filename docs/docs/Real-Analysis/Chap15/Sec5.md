...
##### 15.5.1 证明定理15.5.2<span style="color:blue">（提示：对于(a)，利用[比值判别法](./Sec5.md)；对于(b)和(c)，利用[定理15.1.6](../Chap15/Sec1.md)；对于(d)，利用[定理15.4.1](../Chap15/Sec4.md)；对于(e)，利用(d)；对于(f)，利用(d)并证明当$x$是正数的时候$\exp(x)>1$。你或许会发现[习题7.1.4](./Sec5.md)中的二项式公式可能会很有用）</span>
...
##### 15.5.3 证明命题15.5.4<span style="color:blue">（提示：首先证明当$x$是自然数时的结论，其次证明$x$是整数时的结论，然后证明$x$是有理数时的结论。接着利用“实数是有理数的极限”这一事实取证明关于实数的结论，你或许会发现指数定律（[命题6.7.3](./Sec5.md)）可能会很有用）</span>
...
##### 15.5.5 证明定理15.5.6<span style="color:blue">（提示：对于(a)，利用反函数定理（[定理10.4.2](./Sec5.md)）或者链式法则（[定理10.1.15](./Sec5.md)）；对于(b)，(c)，(d)，利用定理15.5.2和指数定律（[命题6.7.3](./Sec5.md)）；对于(e)，从几何级数公式（[引理7.3.3](./Sec5.md)）入手，并利用[定理15.1.6](../Chap15/Sec1.md)来计算积分）</span>
...
##### 15.5.10 设$f:(0,+\infty)\times\mathbb R\to\mathbb R$是指数函数$f(x,y):=x^y$，证明：$f$是连续的<span style="color:blue">（提示：注意[命题9.4.10](./Sec5.md)和[命题9.4.11](./Sec5.md)只表明了$f$关于每个变量都是连续的，但是这并不能证明你的结论，就像我们在[习题13.2.11](../Chap13/Sec2.md)里面展示的那样；最容易的解决方案是将$f$写成$f(x,y)=\exp(y\ln x)$，利用$\exp$和$\ln$的连续性。你还可以尝试挑战一下自己，在不使用对数函数的前提下完成本题的证明）</span>
...
[实分析 6.7 实数的指数运算 II](./Sec5.md)

[实分析 7.1 有限级数](./Sec5.md)

[实分析 7.3 非负数的和](./Sec5.md)

[实分析 7.5 根值判别法与比值判别法](./Sec5.md)

[实分析 9.4 连续函数](./Sec5.md)

[实分析 10.1 基本定义](./Sec5.md)

[实分析 10.4 反函数及其导数](./Sec5.md)

[实分析 13.2 连续性和乘积空间](../Chap13/Sec2.md)

[实分析 15.1 形式幂级数](../Chap15/Sec1.md)

[实分析 15.4 幂级数的乘法](../Chap15/Sec4.md)