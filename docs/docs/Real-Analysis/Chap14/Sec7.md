...
##### 14.7.2 不假设$f_n'$是连续函数，证明定理14.7.1<span style="color:blue">（提示：于是我们无法使用[微积分基本定理](/docs/Real-Analysis/Chap11/Sec9.md)，但是仍可以使用[平均值定理（推论10.2.9）](/docs/Real-Analysis/Chap10/Sec2.md)。利用该定理证明若$\displaystyle\sup_{x\in X}|f'_n(x)-f'_m(x)|\leq\varepsilon$，则对所有的$x\in[a,b]$都有$|(f_n(x)-f_m(x))-(f_n(x_0)-f_m(x_0))|\leq\varepsilon|x-x_0|$。然后利用这个结论完成定理14.7.1的证明）</span>
